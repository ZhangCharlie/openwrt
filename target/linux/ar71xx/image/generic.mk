...
define Device/airocov-v1
  DEVICE_TITLE := AIROCOV V1 board (16MB flash)
  DEVICE_PACKAGES := kmod-usb-core kmod-usb2 kmod-usb-storage kmod-ath10k ath10k-firmware-qca9888
  BOARDNAME := AIROCOV_V1
  IMAGE_SIZE := 16000k
  MTDPARTS := spi0.0:256(u-boot)ro,64k(u-boot-env)ro,16000k(firmware),64k(art)ro
  SUPPORTED_DEVICES := airocov-v1
  IMAGE/sysupgrade.bin := append-kernel | pad-to $$$$(BLOCKSIZE) | \
	append-rootfs | pad-rootfs | append-metadata | check-size $$$$(IMAGE_SIZE)
endef
TARGET_DEVICES += airocov-v1
